<template>
	<view>
		<view class="spellcon">
			...
			<view class="spell_list">
				<view class="spell_item">
					<view class="label">
						...
						<text class="text">拼物</text>
					</view>
					<view class="content">
						<input v-model="from.name" type="text" class="inputs" placeholder="请输入拼物名称" placeholder-style="color: #848484;" />
				...
				<view class="spell_item">
					<view class="label">
						<image src="/static/images/icon_huser.png" style="width:33rpx;height: 33rpx;"></image>
						<text class="text">人数</text>
					</view>
					<view class="content">
						<input type="text" v-model="from.number" class="inputs" placeholder="请输入拼单人数" placeholder-style="color: #848484;" />
				...
				<view class="spell_item">
					<view class="label">
						<image src="/static/images/icon_price.png" style="width:32rpx;height: 32rpx;"></image>
						<text class="text">拼单价格</text>
					</view>
					<view class="content">
						<input type="text" v-model="from.price" class="inputs" placeholder="请输入拼单价格" placeholder-style="color: #848484;" />
						<view class="icon u-flex" @click="onMod(4)">
							<image src="/static/images/icon_yw.png" style="width:19rpx;height:19rpx;"></image>
						</view>
					</view>
				</view>
				<view class="spell_item">
					...
					<view class="content">
						<input v-model="from.remark" type="text" class="inputs" placeholder="请输入重要信息" placeholder-style="color: #848484;" />
						<view class="icon" @click="onMod(6)">
							<image src="/static/images/icon_yw.png" style="width:19rpx;height:19rpx;"></image>
						</view>
					</view>
				</view>
			...
			<view class="tip">【请告知在广科帮帮平台上看到的】</view>
			...
			<view class="btn_group">
				<view class="commbtn" @click="onSubmit">发布拼单</view>
		...
		methods:{
			...
			onMod(index){
				if(index===1){
					...
					this.modtitle = "拼单人数";
					this.modcon = "温馨提示：请在输入栏输入您本单需拼单的人数，方便与你联系；";
				}else if(index===4){
					this.modtitle = "商品价格";
					this.modcon = "温馨提示：请在输入栏输入您本单需拼单的价格，方便与你联系；";
				}else if(index===5){
					this.modtitle = "商品描述";
					this.modcon = "请在输入框里输入你想要拼单的宝贝的品牌，规格，名称，以及大概价位，有助于更快的找到拼友；";
				}else if(index===6){
					this.modtitle = "商品备注";
					this.modcon = "请在此栏目输入框里输入重要提示，方便用户查阅浏览哦；";
				}
			...
			onSubmit(){
				if(this.from.name=="" || (this.from.phone==""&&this.from.qq==""&&this.from.wx=="") || this.from.number=="" || this.from.price==""){
					uni.showToast({
						title:"请填写完整信息",
						icon:"none"
					})
