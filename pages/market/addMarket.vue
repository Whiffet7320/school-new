		...
		<view class="spellcon">
			...
			<view class="spell_list">
				<view class="spell_item">
					<view class="label">
						<image src="/static/images/icon_rgift.png" style="width:32rpx;height: 32rpx;"></image>
						<text class="text">物品</text>
					</view>
					<view class="content">
						<input v-model="from.name" type="text" class="inputs" placeholder="请输入物品名称" placeholder-style="color: #848484;" />
			...
			<view class="btn_group">
				<view class="commbtn" @click="onSubmit">发布转卖</view>
...
<script>
	...
	export default{
		...
		methods:{
			...
			onSubmit(){
				if(this.from.name=="" || this.from.price=="" || (this.from.phone==""&&this.from.qq==""&&this.from.wx=="") || this.from.desc==""){
					uni.showToast({
						title:"请填写完整信息",
						icon:"none"
					})
